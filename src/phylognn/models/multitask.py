...
class MultiTaskGATNet(BaseGATNet):
    ...
    def _pool_by_time_bins(
        self, x: torch.Tensor, original_features: torch.Tensor, batch: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        pooled = scatter(x, group_index, dim=0, dim_size=batch_size * num_groups, reduce="sum")
        pooled = pooled.view(batch_size, num_groups, -1)

        # Add time bin labels as features
        time_bin_labels = (
            torch.arange(num_groups, device=pooled.device).unsqueeze(0).unsqueeze(2).float()
        )
