...
class TreeToGraphConverter:
    ...
    def convert(
        ...
        graph_attrs : Optional[Dict[str, object]], default=None
            ...
            object. Keys must not match any field generated by this conversion,
            including fields such as `x`, `edge_index`, `time_bin`, or
            `virtual_node_mask`.
        ...
        ValueError
            If any `graph_attrs` key conflicts with a generated graph field.
        ...
        generated_fields = set(data.keys())
        self._validate_graph_attrs(graph_attrs, generated_fields=generated_fields)

        # Attach arbitrary user-defined graph-level metadata after generated
        # fields are finalized so caller values cannot overwrite them.
        if graph_attrs:
    ...
    def _validate_graph_attrs(
        self,
        graph_attrs: Optional[Dict[str, object]],
        *,
        generated_fields: set[str],
    ) -> None:
        """Reject graph-level attributes that would overwrite generated fields."""
        if not graph_attrs:
            return

        conflicting_keys = generated_fields.intersection(graph_attrs)
        if conflicting_keys:
            formatted_keys = ", ".join(f'graph_attrs["{key}"]' for key in sorted(conflicting_keys))
            raise ValueError(
                f"{formatted_keys} conflicts with generated graph fields and cannot be set "
                "by caller metadata."
            )
