...
6. `complete_pipeline.py`
   Complete tree-to-prediction example that consumes the checkpoint written by
...
- `complete_pipeline.py` prints a `Complete pipeline summary`, graph tensor
  shapes, and a prediction value after loading the TOML training checkpoint.
...
- `toml_training_config.toml` defines a representative quickstart setup with a
  tiny non-temporal GAT model and short trainer settings. The invalid
...
- Run `toml_training_config.py` before `complete_pipeline.py` because the
  pipeline intentionally loads the checkpoint generated by the training
  example.
